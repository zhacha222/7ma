...
本项目仅供**技术学习与学术研究**之用，请勿用于任何商业用途或侵犯他人合法权益的行为。

- 使用本项目即表示您已阅读并同意以下内容：
  1. 本项目与「7MA」「7mate」等任何平台方**无任何关联、未获其授权**，名称仅用于识别目标接口。
  2. 使用本项目可能涉及平台的服务条款与当地法律法规的适用问题，**由此产生的一切责任与后果由使用者自行承担**。
  3. 任何因使用本项目造成的账号封禁、资金损失、违约罚金、法律纠纷等，**开发者不予承担任何责任**。
  4. 请在遵守平台规则与当地法律的前提下，合理、审慎地使用。
- 本项目不含任何恶意代码，也不会收集或上传您的任何凭据。
...
本项目采用 **GNU General Public License v3.0（GPL-3.0）** 开源协议，详见 [LICENSE](./LICENSE)。

- **可自由使用、修改、分发**，但任何衍生作品必须同样以 GPL-3.0 协议开源。
- 二次开发或商用前，请务必了解并遵守 GPL-3.0 协议的义务与限制。
- 本工具「按现状」提供，**不附带任何明示或默示的担保**。
